...
import { readableStreamToString, ValidiFiBankLoginError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new ValidiFiBankLoginError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new ValidiFiBankLoginError(e, e.response?.data)
                }
                if (innerError instanceof ValidiFiBankLoginError) {
                    // Got "ValidiFiBankLoginError" from the above try block
                    throw innerError;
                }
